...
import { gmapsMap, gmapsPolyline, gmapsPolygon, GmPosition } from 'v3-gmaps';
...
const items: Ref<GmPosition[]> = ref([]);
...
const polylineOptions: Ref<PolylineOptions> = ref({
	strokeColor: '#0000FF',
	strokeOpacity: 0.8,
	strokeWeight: 10,
});
...
const handlePathChanged = (e: GmPosition[]) => {
	items.value = e;
	log(`@path_changed`);
};
const handlePathsChanged = (e: GmPosition[][]) => {
	items.value = e[0];
	log(`@paths_changed`);
};
