...
// Data
const options: Ref<google.maps.MapOptions> = ref({ ...mapOptions });
...
// Helper function to update options without overwriting the entire object
const updateOptions = (newOptions: Partial<google.maps.MapOptions>) => {
	options.value = { ...options.value, ...newOptions };
};

const handleFullScreenChange = (e: Event) => {
	log(`Full screen controls ${(e.target as HTMLInputElement).checked ? 'on' : 'off'}`);
	updateOptions({ fullscreenControl: (e.target as HTMLInputElement).checked });
};
const handleZoomChange = (e: Event) => {
	log(`Zoom controls ${(e.target as HTMLInputElement).checked ? 'on' : 'off'}`);
	updateOptions({ zoomControl: (e.target as HTMLInputElement).checked });
};
const handleScaleChange = (e: Event) => {
	log(`Scale controls ${(e.target as HTMLInputElement).checked ? 'on' : 'off'}`);
	updateOptions({ scaleControl: (e.target as HTMLInputElement).checked });
};
const handleMapTypeChange = (e: Event) => {
	log(`Map type controls ${(e.target as HTMLInputElement).checked ? 'on' : 'off'}`);
	updateOptions({ mapTypeControl: (e.target as HTMLInputElement).checked });
};
watch(
	() => scrollwheel.value,
	(v) => {
		log(`Scrollwheel ${v ? 'enabled' : 'disabled'}`);
		updateOptions({ scrollwheel: v });
	}
);
watch(
	() => restrict.value,
	(v) => {
		...
		updateOptions({ restriction: v ? { latLngBounds: ausBounds, strictBounds: false } : undefined });
	}
);
watch(
	() => minMaxZoom.value,
	(v) => {
		...
		updateOptions({
			minZoom: v ? 6 : undefined,
			maxZoom: v ? 10 : undefined,
		});
	}
);
</script>
