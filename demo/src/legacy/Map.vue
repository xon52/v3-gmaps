...
import { gmapsMap, GmBounds, GmMapTypeId, GmPosition } from 'v3-gmaps';
import { mapOptions } from './helpers';
import { Ref, ref } from 'vue';
import { log } from '../store';

// Data
const options: Ref<google.maps.MapOptions> = ref({ ...mapOptions, mapTypeControl: true });
const zoom = ref(4);
const center: Ref<GmPosition | undefined> = ref(mapOptions.center);
const bounds: Ref<GmBounds | undefined> = ref();
const type: Ref<GmMapTypeId> = ref('roadmap');

const handleMounted = (e: google.maps.Map) => console.log('Map mounted', e);
const handleBoundsChange = (e: GmBounds | null | undefined) => (e ? (bounds.value = e) : null);
const handleZoomChange = () => log(`Zoom set to "${zoom.value}"`);
const handleTypeChange = () => log(`Type set to "${type.value}"`);
const handleCenterChange = (e: GmPosition | null) => {
	if (e) center.value = e;
	log(`@center_changed event: "${e?.lat.toFixed(2)}, ${e?.lng.toFixed(2)}"`);
};
const handleMapTypeChange = (e: GmMapTypeId | null) => {
	if (e) type.value = e;
